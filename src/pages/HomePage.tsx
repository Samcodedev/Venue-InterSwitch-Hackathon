...
import { formatCurrency, formatDateTime, routeName, shortLocationName } from "@/lib/format";
  ...
  return (
    <div className="max-w-[1400px] mx-auto px-4 md:px-6 grid gap-6 pt-10 pb-16 md:pt-6 md:pb-12">
      <PageIntro
        ...
        description="Track live vehicles, active routes, and upcoming departures from one place."
        actions={
      ...
      <section className="grid grid-cols-1 lg:grid-cols-[1fr_400px] gap-6 items-start">
        ...
        <div className="flex flex-col gap-6">
          <div className="relative overflow-hidden bg-surface border border-surface-border rounded-xl shadow-soft p-6 flex flex-col gap-4">
            <div className="flex items-center justify-between gap-4">
              <div>
                <span className="inline-flex items-center gap-2 text-[0.7rem] uppercase tracking-widest text-teal-deep font-bold leading-none mb-1">Upcoming trips</span>
                <h2 className="text-xl font-bold text-ink">Next departures</h2>
              </div>
            ...
            {loading ? <LoadingScreen message="Loading departures" /> : null}
            {error ? <ErrorState message={error} /> : null}
            {!loading && !error ? (
              <div className="grid gap-3">
                {nextTrips.map((trip) => (
                  <Link key={trip._id} to={`/trips/${trip._id}`} className="flex items-center justify-between gap-4 p-4 rounded-xl bg-white/60 border border-surface-border hover:bg-white/80 hover:translate-y-[-2px] transition-all">
                    <div className="flex flex-col gap-0.5">
                      <strong className="text-ink font-bold">{routeName(trip.route)}</strong>
                      <span className="text-muted text-sm">{formatDateTime(trip.departureTime)}</span>
                    </div>
                    <div className="text-right flex flex-col items-end">
                      <strong className="text-ink font-extrabold">{formatCurrency(trip.price)}</strong>
                      <span className="text-muted text-[0.75rem]">{trip.availableSeats} seats</span>
                    </div>
                  </Link>
                ))}
              </div>
